...
def attn_xplot(ax,par,par_idx,label,short_ylim=False):
	...
	ax.set_xscale('log',nonposx='clip',subsx=(1,2,4,7))
	ax.xaxis.set_minor_formatter(minorFormatter)
	ax.xaxis.set_major_formatter(majorFormatter)
...
def label_sed(ax):
	...
	dely = 7
	lims = [0.1,3]
	ax.plot(lims,[yloc,yloc], lw=lw, alpha=alpha, color=color, zorder=zorder)
	ax.plot([lims[0],lims[0]],[yloc-length,yloc+length], lw=lw, alpha=alpha, color=color, zorder=zorder)
	ax.plot([lims[1],lims[1]],[yloc-length,yloc+length], lw=lw, alpha=alpha, color=color, zorder=zorder)
	ax.text(10**np.average(np.log10(lims)),yloc+dely,r'UV--near-infrared',weight='bold',fontsize=fontsize,ha='center', zorder=zorder)
	...
	ax.plot([lims[0],lims[0]],[yloc-length,yloc+length], lw=lw, alpha=alpha, color=color, zorder=zorder)
	ax.plot([lims[1],lims[1]],[yloc-length,yloc+length], lw=lw, alpha=alpha, color=color, zorder=zorder)
	ax.text(10**np.average(np.log10(lims)),yloc+dely,r'mid-infrared',weight='bold',fontsize=fontsize,ha='center', zorder=zorder)

def plot_sed(ax,ax_res,par_idx,par=None,txtlabel=None,fmt="{:.2e}",init=None, linear_relative=False,
	         xlim = (10**-1.05,10**2), subsx = (1,3), smooth=200):
	...
	for ii,p in enumerate(par):
		if linear_relative:
			ax_res.plot(sps.wavelengths/1e4,(spec_sav[ii] - spec_sav[1])/spec_sav[1]*100,color=colors[ii],lw=lw,alpha=alpha)
			ax_res.set_ylim(-100,100)
			ax_res.set_ylabel(r'percentage change in f$_{\nu}$')
		else:
	...
	if ax is not None:
		...
		ax.set_xlabel(r'wavelength [microns]')
		ax.set_ylabel(r'log($\nu$f$_{\nu}$)')
		ax.set_xscale('log',nonposx='clip',subsx=subsx)
		ax.xaxis.set_minor_formatter(minorFormatter)
		ax.xaxis.set_major_formatter(majorFormatter)
		make_ticklabels_invisible(ax,showx=True)
...
def attenuation_talk_plot():
	...
	par = [-0.4,0.4]
	attn_xplot(ax,par,idx, 'diffuse', short_ylim=True)

	plt.tight_layout()
	plt.savefig('dust_attenuation_xplot.png',dpi=150)
	plt.close()

	figsize = (12,6)
	fig, ax = plt.subplots(1, 2, figsize = figsize)
	plot_sed(ax[0],ax[1], idx, par=par,linear_relative=True)
	plt.tight_layout()
	plt.savefig('dust_attenuation_sed.png',dpi=150)
	plt.close()

	print 1/0

def dustmet_proposal_plot():
	...
	figsize = (8,8)
	fig, ax = plt.subplots(1, 1, figsize = figsize)
	plot_sed(None, ax, idx, par=par,linear_relative=True,subsx=([1]),smooth=10000)

	xt, yt, dy = 0.95,0.93,0.06
	par = [par[0],-1.0,par[1]]
	txtlabel = r'log(Z/Z$_{\odot}$)'
	for ii,p in enumerate(par): 
		ax.text(xt,yt-dy*ii,txtlabel+'='+"{:.2f}".format(p),ha='right',weight='bold',
			    transform=ax.transAxes,color=colors[ii],fontsize=24)

	label_sed(ax)

	plt.tight_layout()
	plt.savefig('metallicity_sed_proposal.png',dpi=150)
	plt.close()
	...
	plt.tight_layout()
	plt.savefig('dust_sed_proposal.png',dpi=150)
	plt.close()

	print 1/0

def dustmet_talk_plot():
	...
	plot_sed(ax[0],ax[1], idx, par=par,linear_relative=True)
	plt.tight_layout()
	plt.savefig('age_sed.png',dpi=150)
	plt.close()
	...
	idx = labels.index('logzsol')
	par = [-1.8,-0.1]
	met_xplot(ax,par,idx)

	plt.tight_layout()
	plt.savefig('metallicity_xplot.png',dpi=150)
	plt.close()
	...
	plt.tight_layout()
	plt.savefig('metallicity_sed.png',dpi=150)
	plt.close()
	...
	figsize = (12,6)
	fig, ax = plt.subplots(1, 2, figsize = figsize)
	plot_sed(ax[0],ax[1], idx, par=par,linear_relative=True,xlim=xlim,subsx=subsx)

	plt.tight_layout()
	plt.savefig('metallicity_sed_limx.png',dpi=150)
	plt.close()
	...
	plot_sed(ax[0],ax[1], idx, par=par,linear_relative=True)
	plt.tight_layout()
	plt.savefig('dust_sed.png',dpi=150)
	plt.close()
	...
	plt.tight_layout()
	plt.savefig('dust_sed_limx.png',dpi=150)
	plt.close()
