...
model_params.append({'name': 'fracmass', 'N': 1,
                        'isfree': True,
                        'init': [],
                        'units': 'Msun',
                        'prior_function': priors.tophat,
...
fit_order = ['fracmass','dust2', 'logzsol', 'dust_index', 'dust1', 'duste_qpah', 'duste_gamma', 'duste_umin']
...
class BurstyModel(sedmodel.SedModel):
    ...
    def prior_product(self, theta):
        ...
        if 'fracmass' in self.theta_index:
            start,end = self.theta_index['fracmass']
            fracmass = theta[start:end]
            if np.sum(fracmass) > 1.0:
                return -np.inf
...
class FracSFH(StepSFHBasis):
    ...
    def all_ssp_weights(self):
        ...
        # over bins.
        bin_masses = np.array(self.params['fracmass'])
        bin_masses = np.append(bin_masses,(1-np.sum(self.params['fracmass'])))*self._time_per_bin
