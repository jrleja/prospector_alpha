...
def ret_cont(alldata,model='Prospector',name='Halpha'):
	'''
	return continuum
	'''
...
def fmt_emline_info(alldata,add_abs_err = True):
	...
	obslines['hdelta_sn_cut'] = 3.0
	obslines['hdelta_eqw_cut'] = 3.0
	...
	####### model absorption properties (Prospector: marginalized + best-fit. MAGPHYS: best-fit)
	...
	hb_lum_pro, hb_eqw_pro, \
	ha_lum_mag, ha_eqw_mag, \
	hb_lum_mag, hb_eqw_mag = [np.zeros(ngals) for i in xrange(8)]
	...
	mag['hbeta_abs'] = hb_lum_mag
	mag['hbeta_eqw'] = hb_eqw_mag
	mag['halpha_abs'] = ha_lum_mag
	mag['halpha_eqw'] = ha_eqw_mag
	...
	hd_lum_mag, hd_eqw_mag = [np.zeros(ngals) for i in xrange(4)]
	for kk, dat in enumerate(alldata):
	...
	mag['hdel_abs'] = hd_lum_mag
	mag['hdel_eqw'] = hd_eqw_mag
	...
	mag['ha'] = ha_magphys
	mag['ha_eqw'] = ha_magphys / (mag['halpha_abs']/mag['halpha_eqw'])
...
def obs_vs_kennicutt_ha(e_pinfo,hflag,outname='test.png',outname_cloudy='test_cloudy.png',
	...
	ax3[0].text(0.04,0.82, r'N = '+str(int(np.sum(keep_idx))), transform = ax3[0].transAxes,horizontalalignment='left')
	...
	ax4[0].text(0.04,0.82, r'N = '+str(int(np.sum(keep_idx))), transform = ax4[0].transAxes,horizontalalignment='left')
...
def bdec_correction(e_pinfo,hflag,outname='test.png',model='Prospectr',
	...
	hdel_obs = e_pinfo['obs']['hdel_eqw'][keep_idx,0]
	hdel_prosp = e_pinfo['prosp']['hdel_eqw'][keep_idx] #bestfit

	if additive:
		ratio = hdel_obs - hdel_prosp
	else:
		ratio = hdel_obs / hdel_prosp
	...
	# use bestfits! XX
	halpha_abs = e_pinfo['prosp']['halpha_eqw'][keep_idx]
	hbeta_abs = e_pinfo['prosp']['hbeta_eqw'][keep_idx]
	halpha_obs = e_pinfo['obs']['f_ha'][keep_idx,0]
	hbeta_obs = e_pinfo['obs']['f_hb'][keep_idx,0]
	ha_continuum_obs = e_pinfo['obs']['ha_obs_cont'][keep_idx]
	hb_continuum_obs = e_pinfo['obs']['hb_obs_cont'][keep_idx]
	...
	sol = minimize(minimize_bdec_corr_eqn, 0.5,
				   args=(hdel_obs, hdel_prosp,
	                     halpha_obs, hbeta_obs,
	                     halpha_abs, hbeta_abs,
	                     ha_continuum_obs, hb_continuum_obs, additive,
	                     bdec_cloudy_marg[:,0]))
	bdec_corrected = bdec_corr_eqn(sol.x, hdel_obs, hdel_prosp, halpha_obs, hbeta_obs, halpha_abs, hbeta_abs, ha_continuum_obs, hb_continuum_obs, additive)

	##### PLOT
	fig, ax = plt.subplots(1,3, figsize = (18.75,6))
	axlim = (0.0,1.0,0.0,1.0)
	...
	ax[0].text(0.05,0.85, r'H$_{\alpha}$,H$_{\beta}$ emission S/N >'+str(int(e_pinfo['obs']['sn_cut'])), transform = ax[0].transAxes)
	ax[0].text(0.05,0.80, r'H$_{\delta}$ absorption S/N >'+str(int(e_pinfo['obs']['hdelta_eqw_cut'])), transform = ax[0].transAxes,horizontalalignment='left')
	ax[0].text(0.05,0.75, r'H$_{\delta}$ absorption S/N >'+str(int(e_pinfo['obs']['hdelta_sn_cut'])), transform = ax[0].transAxes,horizontalalignment='left')
	ax[0].text(0.05,0.70, r'BPT AGN excluded', transform = ax[0].transAxes,horizontalalignment='left')
	ax[0].text(0.05,0.65, r'N = '+str(np.sum(keep_idx)), transform = ax[0].transAxes,horizontalalignment='left')
	...
	ax[1].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat), transform = ax[1].transAxes,horizontalalignment='right')
	ax[1].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off), transform = ax[1].transAxes,horizontalalignment='right')
...
def obs_vs_prosp_balmlines(e_pinfo,hflag,outname='test.png',outname_resid='test.png',model='Prospectr',
	...
	for jj in xrange(2):
		...
		ax[0].text(0.04,0.87, r'S/N (H$\alpha$,H$\beta$) > {0}'.format(int(e_pinfo['obs']['sn_cut'])), transform = ax[0].transAxes,horizontalalignment='left')
		...
		ax[0].text(0.04,0.82, r'N = '+str(int(np.sum(keep_idx))), transform = ax[0].transAxes,horizontalalignment='left')
...
def obs_vs_model_hdelta(e_pinfo,hflag,outname=None,outname_dnplt=None,eqw=False):
	...
	if eqw:
		...
		xtit = [r'observed log(H$_{\delta}$ EQW)', 
		        ...
		        r'observed log(H$_{\delta}$ EQW)',
		        r'observed log(H$_{\delta}$ EQW)']
		ytit = [r'Prospector log(H$_{\delta}$ EQW) [absorption + emission]',
		        r'Prospector log(H$_{\delta}$ EQW) [best-fit]',
		        r'Prospector log(H$_{\delta}$ EQW) [marginalized]',
		        r'MAGPHYS log(H$_{\delta}$ EQW) [best-fit]']
		...
		hdel_prosp_em = e_pinfo['prosp']['hdel_abs_marg'][good_idx] - e_pinfo['prosp']['cloudy_hd'][good_idx]

		hdel_prosp = np.log10(e_pinfo['prosp']['hdel_abs'][good_idx])
		hdel_mag = np.log10(e_pinfo['mag']['hdel_abs'][good_idx])
		xtit = [r'observed log(H$_{\delta}$)', 
		        r'observed log(H$_{\delta}$)',
		        r'observed log(H$_{\delta}$)',
		        r'observed log(H$_{\delta}$)']
		ytit = [r'Prospector log(H$_{\delta}$) [absorption + emission]',
		        r'Prospector log(H$_{\delta}$) [best-fit]',
		        r'Prospector log(H$_{\delta}$) [marginalized]',
		        r'MAGPHYS log(H$_{\delta}$) [best-fit]']
	...
	for ii in xrange(len(labels)):
		for kk in xrange(len(hflag)):
			...
			ax[2].errorbar(np.log10(pl_hdel_obs), np.log10(pl_hdel_prosp), xerr=errs_obs, yerr=errs_pro, linestyle=' ', **pdict)
			ax[3].errorbar(np.log10(pl_hdel_obs), pl_hdel_mag, xerr=errs_obs, linestyle=' ', **pdict)
			...
			if eqw:
				...
				ax2[2].errorbar(pl_hdel_mag, dn4000_mag[plt_idx], linestyle=' ',**pdict)
	...
	ax[0].text(0.04,0.82, r'N = '+str(int(np.sum(good_idx))), transform = ax[0].transAxes,horizontalalignment='left')
	ax[0].set_xlabel(xtit[0])
	ax[0].set_ylabel(ytit[0])
	...
	ax[1].set_xlabel(xtit[1])
	ax[1].set_ylabel(ytit[1])
	off,scat = threed_dutils.offset_and_scatter(np.log10(hdel_obs[:,0]), hdel_prosp,biweight=True)
	...
	ax[2].set_xlabel(xtit[2])
	ax[2].set_ylabel(ytit[2])
	off,scat = threed_dutils.offset_and_scatter(np.log10(hdel_obs[:,0]), np.log10(hdel_prosp_marg[:,0]),biweight=True)
	ax[2].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat) + ' dex', transform = ax[2].transAxes,horizontalalignment='right')
	ax[2].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off) + ' dex', transform = ax[2].transAxes,horizontalalignment='right')
	ax[2].axis(plotlim)
	ax[2].plot([min,max],[min,max],linestyle='--',color='0.1',alpha=0.8)

	ax[3].set_xlabel(xtit[3])
	ax[3].set_ylabel(ytit[3])
	off,scat = threed_dutils.offset_and_scatter(np.log10(hdel_obs[:,0]), hdel_mag,biweight=True)
	ax[3].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat), transform = ax[3].transAxes,horizontalalignment='right')
	ax[3].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off), transform = ax[3].transAxes,horizontalalignment='right')
	ax[3].axis(plotlim)
	ax[3].plot([min,max],[min,max],linestyle='--',color='0.1',alpha=0.8)
...
def obs_vs_model_dn(e_pinfo,hflag,outname=None):
	...
	dn4000_mag = e_pinfo['mag']['dn4000'][dn_idx]
	...
	for ii in xrange(len(labels)):
		for kk in xrange(len(hflag)):
			...
			pl_dn4000_mag = dn4000_mag[plt_idx]
			...
			ax[0].errorbar(pl_dn4000_obs, pl_dn4000_prosp, yerr=errs_pro, linestyle=' ', **pdict)
			ax[1].errorbar(pl_dn4000_obs, pl_dn4000_mag, linestyle=' ', **pdict)

	ax[0].set_xlabel(r'observed D$_n$(4000)')
	ax[0].set_ylabel(r'Prospector D$_n$(4000)')
	ax[0] = threed_dutils.equalize_axes(ax[0], dn4000_obs, dn4000_prosp)
	off,scat = threed_dutils.offset_and_scatter(dn4000_obs, dn4000_prosp,biweight=True)
	ax[0].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat), transform = ax[0].transAxes,horizontalalignment='right')
	ax[0].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off), transform = ax[0].transAxes,horizontalalignment='right')
	ax[0].text(0.04,0.9, r'N = '+str(int(np.sum(dn_idx))), transform = ax[0].transAxes,horizontalalignment='left')

	ax[1].set_xlabel(r'observed D$_n$(4000)')
	ax[1].set_ylabel(r'MAGPHYS D$_n$(4000)')
	ax[1] = threed_dutils.equalize_axes(ax[1], dn4000_obs, dn4000_mag)
...
def obs_vs_model_bdec(e_pinfo,hflag,outname1='test.png',outname2='test.png'):
	...
	#### MAIN FIGURE ERRATA
	ax1[0].text(0.04,0.87, r'S/N (H$\alpha$,H$\beta$) > {0}'.format(int(e_pinfo['obs']['sn_cut'])), transform = ax1[0].transAxes,horizontalalignment='left')
	ax1[0].text(0.04,0.92, r'EQW (H$\alpha$,H$\beta$) > {0} $\AA$'.format(int(e_pinfo['obs']['eqw_cut'])), transform = ax1[0].transAxes,horizontalalignment='left')
	ax1[0].text(0.04,0.82, r'N = '+str(int(np.sum(keep_idx))), transform = ax1[0].transAxes,horizontalalignment='left')
	...
	ax1[0].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat), transform = ax1[0].transAxes,horizontalalignment='right')
	ax1[0].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off), transform = ax1[0].transAxes,horizontalalignment='right')
	...
	ax1[1].text(0.96,0.05, 'biweight scatter='+"{:.2f}".format(scat), transform = ax1[1].transAxes,horizontalalignment='right')
	ax1[1].text(0.96,0.1, 'mean offset='+"{:.2f}".format(off), transform = ax1[1].transAxes,horizontalalignment='right')
...
def obs_vs_prosp_sfr(e_pinfo,hflag,outname='test.png'):
	...
	ax[0].text(0.04,0.87, r'S/N (H$\alpha$,H$\beta$) > '+str(int(e_pinfo['obs']['sn_cut'])), transform = ax[0].transAxes,horizontalalignment='left')
