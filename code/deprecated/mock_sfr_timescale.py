...
def plot_mass_resid(alldata,outfolder=None):
	...
	true_slope = np.array([dat['truths']['truths'][idx_slope] for dat in alldata])
	idx_mass = pars == 'mass'
	true_mass =  np.log10(np.array([dat['truths']['truths'][idx_mass] for dat in alldata]))
	...
	plt.subplots_adjust(wspace=0.25,hspace=0.3,bottom=0.13,left=0.1,right=0.9,top=0.87)
	ax = np.ravel(axes)

	ax[0].errorbar(true_slope,ratio,yerr,fmt='o',alpha=0.8,color='#1C86EE')
	ax[0].set_xlabel('true sf_tanslope')
	ax[0].set_ylabel('log(M$_{\mathrm{true}}$/M$_{\mathrm{fit}}$)')
	...
	ax[0].axhline(0.0, ls="dashed", color='k',linewidth=1.5)
	ax[0].axvline(0.0, ls="dashed", color='k',linewidth=1.5)

	ax[1].errorbar(halfmass_ratio,ratio,yerr=yerr,xerr=halfmass_ratio_err,fmt='o',alpha=0.8,color='#1C86EE')
	ax[1].set_xlabel(r'log(t$_{\mathrm{half,true}}$/t$_{\mathrm{half,fit}}$)')
	ax[1].set_ylabel('log(M$_{\mathrm{true}}$/M$_{\mathrm{fit}}$)')
	...
	'''
	ax[1].errorbar(true_halfmass,ratio,yerr,fmt='o',alpha=0.8,color='#1C86EE')
	ax[1].set_xlabel(r'true log(t$_{\mathrm{half}}$)')
	ax[1].set_ylabel('log(M$_{\mathrm{true}}$/M$_{\mathrm{fit}}$)')
