...
def make_mocks(basename,outname=None,add_zp_err=False):
	...
	for ii in xrange(nparams):
		...
		if parnames[ii] == 'mass':
			min,max = 10,11
			for kk in xrange(ngals_per_model): testparms[kk,ii] = 10**(random.random()*(max-min)+min)

		#### choose reasonable amounts of dust ####
		elif parnames[ii] == 'dust2':
			min = 0.0
			max = 0.5
			for kk in xrange(ngals_per_model): testparms[kk,ii] = random.random()*(max-min)+min
		...
		#### dust1 must match the dust1/dust2 prior #### 
			...
			min = dust2 * 0.5
			max = dust2 * 2.0
			for kk in xrange(ngals_per_model): testparms[kk,ii] = random.random()*(max[kk]-min[kk])+min[kk]
