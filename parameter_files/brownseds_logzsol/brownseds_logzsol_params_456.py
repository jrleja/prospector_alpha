...
def translate_filters(bfilters, full_list = False):
    ...
    if full_list:
        return translate.values(), translate_pfsps.values()
    else:
        return np.array([translate[f] for f in bfilters]), np.array([translate_pfsps[f] for f in bfilters])

def load_obs_mock(filename, objnum, zperr=False):
    """
    ...
    wave_effective = calc_lameff_for_fsps(translate_filters(None, full_list=True)[0])
