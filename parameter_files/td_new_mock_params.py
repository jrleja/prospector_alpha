...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/td_new_mock/1',
              'nofork': True,
              # dynesty params
...
def load_obs(mock_gal=None, datdir=None, runname=None, snr=10, **extras):
    ...
    # load sample results from 'random' galaxy (in order, in AEGIS)
    outname = APPS+'/prospector_alpha/results/td_new/'+mock_gal.split('_')[0]+'/'+mock_gal
    sres, _, _, _ = load_prospector_data(outname)
    
    # generate mocks by sampling from posteriors if they exist
    # otherwise sample randomly from priors
    if sres is not None:
        idx = np.random.choice(np.arange(sres['chain'].shape[0]),size=1,p=sres['weights'])[0]
        theta = sres['chain'][idx,:]
    else:
        theta = []
        for par in mod.free_params: theta += mod._config_dict[par]['prior'].sample().tolist()
        theta = np.array(theta)

    # now that we have the object name, use it to get the field
    # and load filternames from the photometric file
    photname = datdir + mock_gal.split('_')[0] + '_' + runname + '.cat'
    with open(photname, 'r') as f:
        hdr = f.readline().split()
    filters = observate.load_filters([f[2:] for f in hdr if f[0:2] == 'f_'])
    mock = {'wavelength': None,'filters':filters}

    # And build the mock
    spec, phot, _ = mod.mean_model(theta, mock, sps=sps)
    # Now store some output
    mock['true_spectrum'] = spec.copy()
    mock['true_maggies'] = phot.copy()
    mock['mock_params'] = copy.deepcopy(mod.params)
    ...
    mock['mock_snr'] = snr
    mock['wave_effective'] = np.array([filt.wave_effective for filt in mock['filters']])

    # we follow logic from the main branch to create a mask
    # mask anything touching or bluewards of Ly-a
    phot_mask = np.ones(len(phot), dtype=bool)
    zred = mod.params['zred']
    wavemax = np.array([f.wavelength[f.transmission > (f.transmission.max()*0.1)].max() for f in filters]) / (1+zred)
    wavemin = np.array([f.wavelength[f.transmission > (f.transmission.max()*0.1)].min() for f in filters]) / (1+zred)
    filtered = [1230]
    for f in filtered: phot_mask[(wavemax > f) & (wavemin < f)] = False
    phot_mask[wavemin < 1200] = False
    mock['phot_mask'] = phot_mask
...
def load_model(mock_gal=None, datdir=None, runname=None, agelims=[], zred=None, alpha_sfh=0.2, **extras):
    ...
    if zred is None:
        datname = datdir + mock_gal.split('_')[0] + '_' + runname + '.dat'
        dat = ascii.read(datname)
        idx = dat['phot_id'] == int(mock_gal.split('_')[-1])
