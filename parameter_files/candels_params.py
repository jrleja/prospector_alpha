...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/candels/00449',
              'nofork': True,
              # dynesty params
              ...
              'datloc':APPS+'/prospector_alpha/data/CANDELS_GDSS_workshop.dat',
              'runname': 'candels',
              'objname':'00449'
              }
############
...
def load_obs(objname=None, datloc=None, err_floor=0.05, **extras):
    ...
    zred = dat['zz'][obj_idx]
    ofilters = observate.load_filters([f+'_candels' for f in filters])
    ...
    """
    mask = obs['phot_mask']
    plt.errorbar(obs['wave_effective'][mask],obs['maggies'][mask],yerr=obs['maggies_unc'][mask],fmt='o',ms=5,linestyle=' ',color='red')
    plt.xscale('log')
    plt.yscale('log')
...
fit_order = ['massmet','z_fraction', 'dust2', 'dust_index', 'dust1_fraction', 'gas_logz']
...
def load_model(objname=None, datloc=None, agelims=[], zred=None, alpha_sfh=0.2, **extras):
