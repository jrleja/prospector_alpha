...
run_params = {'verbose':True,
              'debug': False,
              'outfile': os.getenv('APPS')+'/threedhst_bsfh/results/nicholl/nicholl',
              'nofork': True,
              # Optimizer params
              'ftol':0.5e-5, 
              'maxfev':5000,
              ...
              # Data info
              'datname':os.getenv('APPS')+'/threedhst_bsfh/data/nicholl.dat',
              'objname':'galaxy',
              }
...
def load_obs(datname='', **extras):
    ...
    with open(datname, 'r') as f:
        ...
        mag = np.array(f.readline().split(),dtype=float)
        magerr = np.array(f.readline().split(),dtype=float)
    ...
    fluxerr = np.clip(fluxerr, flux*0.05, np.inf)

    # build output dictionary
    obs['filters'] = observate.load_filters(fnames)
    obs['wave_effective'] = np.array([filt.wave_effective for filt in obs['filters']])
    obs['phot_mask'] = np.ones_like(fnames,dtype=bool)
    obs['maggies'] = flux
    obs['maggies_unc'] =  fluxerr
    obs['wavelength'] = None
    obs['spectrum'] = None
