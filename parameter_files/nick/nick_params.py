...
def load_obs(photname='', **extras):
    ...
    mag = np.array([f for f in dat[mag_fields][()]])
    magunc  = np.array([f for f in dat[magunc_fields][()]])

    # convert to maggies from nanomaggies
    flux = mag/1e9
    unc = magunc/1e9
    phot_mask = flux != 0
    ...
    fnames = [f[2:] for f in mag_fields]
    fnames = [f if 'vista' not in f else 'u'+f.lower()+'_cosmos' for f in fnames]
