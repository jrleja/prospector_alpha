...
run_params = {'verbose':True,
              'debug': False,
              'outfile': os.getenv('APPS')+'/prospector_alpha/results/brownseds_np/brownseds_np',
              'nofork': True,
              # Optimizer params
              'ftol':0.5e-5, 
              'maxfev':5000,
...
def load_model(objname='',datname='', agelims=[], **extras):
    ...
    model_params[n.index('sfr_fraction')]['prior'].params['mini'] = np.full(ncomp-1,0.0)
    model_params[n.index('sfr_fraction')]['prior'].params['maxi'] = np.full(ncomp-1,1.0)
