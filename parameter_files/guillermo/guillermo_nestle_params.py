...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/guillermo_nestle/guillermo_nestle',
              'nofork': True,
              # Optimizer params
              'ftol':0.5e-5, 
              'maxfev':5000,
              ...
              'cat':APPS+'/prospector_alpha/data/gds14876_allphotometry.cat',
              'objinfo': APPS+'/prospector_alpha/data/allfilters.dat'
              }
...
model_params.append({'name': 'logmass', 'N': 1,
                        ...
                        'init': 10.0,
                        'units': 'Msun',
                        'prior': priors.TopHat(mini=10.0, maxi=13.0)})
...
model_params.append({'name': 'dust_index', 'N': 1,
                        ...
                        'disp_floor': 0.15,
                        'units': '',
                        'prior': priors.TopHat(mini=-0.7, maxi=0.4)})
...
#### redefine model for nestled sampling
class NestleModel(sedmodel.SedModel):
    ...
    def prior_product(self, theta, **extras):
        """as they say, it's all in the unit cube
        """
...
class FracSFH(FastStepBasis):
    ...
    def get_spectrum(self, outwave=None, filters=None, peraa=False, **params):
        ...
        will need to rewrite when the homogenize_bases branch is input
...
def load_model(objname=None, datname=None, agelims=[], **extras):
    ...
    #### CREATE MODEL
    model = NestleModel(model_params)

    return model

model_type = NestleModel
