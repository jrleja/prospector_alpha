...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/mock_timebin_lm/1',
              'nofork': True,
              # dynesty params
              'nested_bound': 'multi', # bounding method
              'nested_sample': 'rwalk', # sampling method
              'nested_walks': 60, # MC walks
...
def load_model(nbins_sfh=7,sigma=0.3,df=2.,agelims=None,objname=None, **extras):
    ...
    model_params[n.index('mass')]['init'] = np.full(nbins_sfh,1e6)
    model_params[n.index('mass')]['prior'] = priors.LogUniform(mini=np.full(nbins_sfh,1e3), maxi=np.full(nbins_sfh,1e12))
    model_params[n.index('logmasses')]['N'] = nbins_sfh
    model_params[n.index('logmasses')]['init'] = np.full(nbins_sfh,6)
    model_params[n.index('logmasses')]['prior'] = priors.TopHat(mini=np.full(nbins_sfh,3), maxi=np.full(nbins_sfh,12))
