...
def logmass_to_masses(logmass=None, agebins=None, **extras):
    nbins = agebins.shape[0]
    return np.full(nbins,10**logmass / nbins)
...
def logsfr_ratios_to_agebins(logsfr_ratios=None, tuniv=None, **extras):
    ...
    dt1 = tuniv[0] / (1 + np.sum([np.prod(sfr_ratios[:(i+1)]) for i in range(n_ratio)]))
...
def load_model(objname=None, datdir=None, runname=None, zred=None, nbins_sfh=7, **extras):
    ...
    model_params[n.index('logsfr_ratios')]['prior'] = SFR_Ratio(mean=np.full(nbins_sfh-1,0.0),sigma=np.full(nbins_sfh-1,0.25))
