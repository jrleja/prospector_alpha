...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/mo/mo',
              'nofork': True,
              # dynesty params
...
def load_obs(photname=None, objname=None, err_floor=0.05, **extras):
    ...
    phot_mask = (flux != unc) & (flux != -99) & (unc > 0)
    maggies = flux/(1e10)
    ...
    zred = 1.882
    ofilters = observate.load_filters([trans_phot[f] for f in filters])
...
def load_model(objname=None, datdir=None, runname=None, agelims=[], zred=1.882, nbins_sfh=7, sigma=0.3,df=2, **extras):
