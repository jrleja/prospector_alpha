...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/td_fir_snr/AEGIS_13',
              'nofork': True,
              # dynesty params
...
def return_snr(snr_key, old_method=False):

    snr = np.linspace(0.1,20,num=100)
    return snr[int(snr_key)-1]

def load_obs(objname=None, datdir=None, runname=None, err_floor=0.05, zperr=True, no_zp_corrs=False,snr_key=1, **extras):
    ...
    ### add Herschel bands
    snr = return_snr(snr_key)
    hflux = np.array([5.57686770e-09,1.03318207e-08,2.99350118e-08,5.02260245e-08,4.98272207e-08])
    hunc = hflux / snr
    maggies = np.concatenate((maggies,hflux))
    maggies_unc = np.concatenate((maggies_unc,hunc))
    phot_mask = np.concatenate((phot_mask,np.ones_like(hflux,dtype=bool)))
    filters = np.concatenate((filters,np.array(['herschel_pacs_100','herschel_pacs_160','herschel_spire_250','herschel_spire_350','herschel_spire_500'])))
...
model_params.append({'name': 'duste_gamma', 'N': 1,
                        ...
                        'disp_floor': 0.15,
                        'units': None,
                        'prior': priors.TopHat(mini=0.0, maxi=0.7)})
...
fit_order = ['massmet','z_fraction', 'dust2', 'dust_index', 'dust1_fraction', 'fagn', 'agn_tau', 'gas_logz','duste_qpah','duste_gamma','duste_umin']
