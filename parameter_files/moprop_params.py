...
run_params = {'verbose':True,
              'debug': False,
              'outfile': APPS+'/prospector_alpha/results/moprop/moprop',
              'nofork': True,
              # dynesty params
...
def load_obs(objname=None, err_floor=0.05, **extras):
    ...
    # open data
    dloc = APPS+'/prospector_alpha/data/CLASH_fluxes.dat'
    dat = np.genfromtxt(dloc,dtype=[('band', '|S15'), ('lameff', 'f8'), ('flux', 'f8'),('fluxerr','f8')])

    # define filters we're fitting
    if objname == '1':
        filters = ['F125W', 'F160W']
    if objname == '2':
        filters = ['F105W', 'F110W', 'F125W', 'F140W', 'F160W']
    if objname == '3':
        filters = ['F475W', 'F606W', 'F110W', 'F125W', 'F160W']
    if objname == '4':
        filters = dat['band'][1:]
    idx = np.in1d(dat['band'],filters)

    ### extract filters, fluxes, errors
    # fluxes are in 1.e-18 erg/cm2/s/AA, convert to maggies!
    filters = dat['band'][idx]
    conversion_factor = 1e-18 * (dat['lameff'][idx]**2/3e18) * (u.erg / u.cm**2).to(u.Jansky) / 3631
    maggies = dat['flux'][idx]*conversion_factor
    maggies_unc = dat['fluxerr'][idx]*conversion_factor
...
def load_model(objname=None, datdir=None, runname=None, agelims=[], zred=2.1478, nbins_sfh=7, sigma=0.3,df=2, **extras):
